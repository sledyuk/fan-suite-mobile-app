...
| 17 | Icons | Iconsax (Vuesax) via `iconsax-react-native`, Linear by default, Bold when filled — the family the Figma uses. One `components/icons.tsx` module keeps the app's semantic names; two custom SVG glyphs (check, close) that Iconsax lacks | Match the design system; swap-in-one-place if the team ships its own SVGs |
