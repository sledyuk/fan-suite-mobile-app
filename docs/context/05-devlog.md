...
- Seeding consistency: rows' last messages are now real. Fan-last / delivered / seen rows become the *tail* of the thread, appended lazily by the mock server on first open (`tailFor`), so seeding stays instant (eager injection wrote 8 × 6.7 MB synchronously). Failed / sending rows are genuine outbox items created inside one MobX action (a race let the "failed" seed get sent). `OutboxStore.enqueue` takes `{ createdAt }` and returns the observable item (mutating the plain input was lost).
- Drainer queue blocking is by outbox position, not timestamp (same-ms ties). Gifts removed from generated history (not a feature). List pins to the end after first layout (estimated sizes) so the newest rows are visible on open.
- Failed bubble redesigned after checking iMessage / WhatsApp / Telegram: bubble keeps its look, red "!" badge beside it, "Not delivered · Tap to retry / Tap to subscribe / <reason>" caption. Tap = retry (or paywall), swipe right = Delete (RNGH swipeable). Thread header: "‹ Chats" back label, centred title, glass bug button. Own sends and first paint defer scroll-to-end past layout.
