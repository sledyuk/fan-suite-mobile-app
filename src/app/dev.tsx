...
type Run = 'seed' | 'reset' | 'offline' | 'online' | 'send' | 'inject' | 'drop' | 'buggy' | 'fail' | 'outcome' | 'retry' | 'discard' | 'forgetPlan';
  ...
  useEffect(() => {
    ...
    runInAction(() => {
      switch (p.run) {
        ...
        // Drops the local entitlement but keeps the mock store's receipt, so Restore has something to find.
        case 'forgetPlan': root.billing.clear(); break;
